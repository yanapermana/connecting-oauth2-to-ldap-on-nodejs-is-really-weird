...
var client = ldap.createClient({
	url: 'ldap://themostweirdestldapintheworld'
});
...
var getUser = function(uname, passwd, callback) {
	var isConnect = function(err) {
		...
		if(err === null){
			console.log(err);
			callback(false, {id: '1337', username: uname, password: passwd});
		} else{
			console.log(err);
			callback(false, undefined);
		}
	}
	client.bind('cn='+uname, passwd, isConnect);
};
